...
$data['categorias'] = $this->Crud->getRows($data_crud);
  ...
  */
   ...
   if ($this->input->post()) {
     ...
     $data_form = array(
      "id_tblhoteles_categoria" => $post['id_tblhoteles_categoria'],
       "nombre" => $post['nombre'],
    ...
    //Agregar
     if($tipo == 'C'){
       $this->db->insert('tblhoteles', $data_form);
     ...
     if ($tipo == 'E') {
       ...
       $this->db->update('tblhoteles', $data_form);
       $Tour_id = $post['id'];
