  ...
  <head>
    ...
                    type:  'post',
                    success:  function (response) {
                       window.location.href='listado_paginas.php'; 
                    }
              });
          });
...
<div class="container">
 ...
 <div class="collapse navbar-collapse" id="bs-example-navbar-collapse-1">
  ...
  <button type="button" id="nuevo" class="btn btn-primary" aria-label="Left Align">
                <span class="glyphicon glyphicon-plus" aria-hidden="true"></span> Nueva página
  </button>
  <div class="row col-lg-offset-5">
    <h3>Listado de páginas</h3></div>
  <div class="row">
      <table class="table-striped table table-condensed table-responsive" id="listado">
        ...
        <tbody>
    <?php 
     $sql="SELECT * from tblpagina Where estado != 0";
     $res=mysql_query($sql, $conection);
     while($row=  mysql_fetch_array($res))
     {
      $urlPage = MURL . 'p/' . $row['url_key'];
        echo '<tr id="'.$row['id'].'">
         <td>'.$row['titulo'].'</td>
         <td><a href="'.$urlPage.'" target="_blank">'.$urlPage.'</a></td>
